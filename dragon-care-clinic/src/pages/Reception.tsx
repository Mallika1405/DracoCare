...
const Reception = () => {
  ...
  const addBotMessage = (text: string, askingForLabs: boolean) => {
    ...
    // Show upload widget whenever backend signals Alex is asking about labs
    if (askingForLabs && !labsUploaded) {
  ...
  const sendToBackend = async (message: string) => {
    ...
    try {
      ...
      if (data.ready_for_consultation) {
        setTimeout(() => {
          navigate("/consultation", { state: { sessionId: SESSION_ID } });
        }, 2000);
      }
    } catch {
      setIsTyping(false);
  ...
  const handleSend = () => {
    ...
    // If user types something while upload panel is visible, hide it and keep going
  ...
  const handleFileUpload = async (file: File) => {
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        // Now notify Alex via the dedicated endpoint so she gets context
        setIsTyping(true);
        const notifyRes = await fetch(`${API}/notify-lab-upload`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            message: data.summary || "Lab results uploaded successfully.",
  ...
  const skipUpload = () => {
    ...
    setUploadStatus("");
    setMessages(prev => [...prev, {
      text: "I don't have any lab results to upload right now",
      isUser: true,
    }]);
      ...
      >
        <div className="max-w-sm mx-auto space-y-4">
          <AnimatePresence initial={false}>
            {messages.map((msg, i) => (
              ...
              >
                ...
                >
                  ...
                  <div
                    className={`px-4 py-2.5 rounded-2xl text-sm font-body font-semibold leading-relaxed shadow-sm ${
                      msg.isUser
            ...
            {/* Inline upload card — appears in chat flow when Alex asks */}
            {showUpload && !labsUploaded && (
              ...
              >
                ...
                />
                <div
                  className={`max-w-[80%] p-4 rounded-2xl rounded-bl-sm border-2 border-dashed transition-all ${
                    isDragging
                ...
                >
                  ...
                  <p className="text-[10px] text-muted-foreground/40 mt-2">
                    or drag & drop here • Alex will read it and skip questions already answered
                  </p>
                  <input
                    ref={fileInputRef}
                    type="file"
